        ...
        <>
            ...
            <div className='border border-[#363636] rounded-b-md'>
                <div className='flex flex-row items-center justify-between mr-1'>
                    ...
                    <button onClick={() => { setShow(pre => !pre) }}><FontAwesomeIcon icon={icon} className='size-4 p-1' /></button>
                </div>
                {show && <div className='w-[100%] flex justify-center bg-[#101010] rounded-md p-3 border-2 text-xs border-[#555555]'>
                    {big ? showMore ? <div>
                        {props.data.comment}
                        <button onClick={() => { setShowMore(false) }} className='underline'> Read Less</button>
                    </div> : <div>
                        {toShow}
                        <button onClick={() => { setShowMore(true) }}  className='underline'>Read More</button>
                    </div> : <div>
                        {props.data.comment}
                    </div>
                    }
